...
def create_customer_proposal_pdf(house_price, solar_price, total_price, project_details, notes, customer_info):
    ...
    if project_details['structure_type'] == 'Light Steel':
        ...
        building_structure_table_data.append([Paragraph('<b>Exterior Walls:</b>', styles['NormalBilingual']), Paragraph(EXTERIOR_WALLS_DESCRIPTION_EN_GR, styles['NormalBilingual'])])
    else: # Heavy Steel
        ...
        building_structure_table_data.append([Paragraph('<b>Steel Structure Details:</b>', contract_subheading_style), Paragraph(HEAVY_STEEL_BUILDING_STRUCTURE_EN_GR, styles['NormalBilingual'])]) # Changed to contract_subheading_style to avoid warning
        building_structure_table_data.append([Paragraph('<b>Roof:</b>', styles['NormalBilingual']), Paragraph(ROOF_DESCRIPTION_EN_GR, styles['NormalBilingual'])])
...
def create_internal_cost_report_pdf(cost_breakdown_df, financial_summary_df, profile_analysis_df, project_details, customer_info):
    ...
    if wall_plasterboard_costs:
        elements.extend(create_cost_table(wall_plasterboard_costs, "Duvarlar (Alçıpan, Kaplamalar ve Yalıtım) Detayları"))
...
def create_sales_contract_pdf(customer_info, house_sales_price, solar_sales_price, project_details, company_info):
    ...
    if project_details['structure_type'] == 'Light Steel':
        ...
        construction_materials_table_data.append([Paragraph('<b>Exterior Walls:</b>', contract_subheading_style), Paragraph(EXTERIOR_WALLS_DESCRIPTION_EN_GR, styles['NormalBilingual'])]) # Changed from normal style to bilingual
    else: # Heavy Steel
        ...
        construction_materials_table_data.append([Paragraph('<b>Steel Structure Details:</b>', contract_subheading_style), Paragraph(HEAVY_STEEL_BUILDING_STRUCTURE_EN_GR, styles['NormalBilingual'])]) # Changed to normal style to bilingual
        construction_materials_table_data.append([Paragraph('<b>Roof:</b>', contract_subheading_style), Paragraph(ROOF_DESCRIPTION_EN_GR, styles['NormalBilingual'])]) # Changed to normal style to bilingual
    ...
    coverings_table_data = [
        [Paragraph("<b>Floor Coverings:</b>", contract_subheading_style), Paragraph(f"Used for floor coverings: {project_details['floor_covering_type']}.", contract_normal_style)],
    ...
    if project_details['solar']:
        additional_features_table_data.append([Paragraph("<b>Solar System:</b>", contract_subheading_style), Paragraph(f"Yes ({project_details['solar_kw']} kW)", contract_normal_style)]) # Corrected formatting
    if project_details['wheeled_trailer_included']:
        additional_features_table_data.append([Paragraph("<b>Wheeled Trailer:</b>", contract_subheading_style), Paragraph(f"Yes ({format_currency(project_details['wheeled_trailer_price'])})", contract_normal_style)]) # Corrected formatting
...
def run_streamlit_app():
    ...
    # Aether Living'e özel ek UI elementleri için varsayılanlar (artık manuel olarak kontrol edilecek)
    ...
    if st.session_state.aether_package_choice == 'Aether Living | Loft Standard (BASICS)':
        ...
        # Elite pakette Premium'daki ek donanımlar da True olur
        st.session_state.bedroom_set_option_val = True
        st.session_state.brushed_granite_countertops_option_val = True
        st.session_state.terrace_laminated_wood_flooring_option_val = False # Elite'de beton zemin varsa teras çam zemin default kapalı

    # Project Details içindeki UI elemanları doğrudan session_state'ten okur
    ...
    with col1:
        ...
        # Yapı Tipine göre Alçıpan seçimini devre dışı bırakma
    ...
    with col_floor_mats[0]:
        st.session_state.skirting_count_val = st.number_input(f"Süpürgelik ({FIYATLAR['skirting_meter_price']}€/m) Uzunluğu (m):", value=st.session_state.skirting_count_val, step=0.1, min_value=0.0, disabled=floor_insulation_material_disabled, key="skirting_input")
    with col_floor_mats[1]:
        st.session_state.laminate_flooring_m2_val = st.number_input(f"Laminat Parke 12mm ({FIYATLAR['laminate_flooring_m2_price']}€/m²) Alanı (m²):", value=st.session_state.laminate_flooring_m2_val, step=0.1, min_value=0.0, disabled=floor_insulation_material_disabled, key="laminate_flooring_input")
    with col_floor_mats[2]:
        st.session_state.under_parquet_mat_m2_val = st.number_input(f"Parke Altı Şilte 4mm ({FIYATLAR['under_parquet_mat_m2_price']}€/m²) Alanı (m²):", value=st.session_state.under_parquet_mat_m2_val, step=0.1, min_value=0.0, disabled=floor_insulation_material_disabled, key="under_parquet_mat_input")
    
    col_floor_mats2 = st.columns(3)
    with col_floor_mats2[0]:
        st.session_state.osb2_18mm_count_val = st.number_input(f"OSB2 18mm/Beton Panel ({FIYATLAR['osb2_18mm_piece_price']}€/adet) Adet:", value=st.session_state.osb2_18mm_count_val, min_value=0, disabled=floor_insulation_material_disabled, key="osb2_input")
    with col_floor_mats2[1]:
        st.session_state.galvanized_sheet_m2_val = st.number_input(f"5mm Galvanizli Sac ({FIYATLAR['galvanized_sheet_m2_price']}€/m²) Alanı (m²):", value=st.session_state.galvanized_sheet_m2_val, step=0.1, min_value=0.0, disabled=floor_insulation_material_disabled, key="galvanized_sheet_input")
    ...
    st.session_state.transportation_input_val = st.checkbox("Nakliye Dahil Et (350€)", value=st.session_state.transportation_input_val, key="transportation_checkbox")
